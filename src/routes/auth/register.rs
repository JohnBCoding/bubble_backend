...
#[post("/register")]
pub async fn register_user(session: Session, user: Json<UserLogin>) -> HttpResponse {
    let firebase_key =
        ...
        Ok(res) => {
            if res.status() == 200 {
                ...
                if let Ok(login) = login_result {
                    // Set cookie tokens
                    let _ = session.insert("id_token", login.token.clone());
                    let _ = session.insert("refresh_token", login.refresh_token.clone());
